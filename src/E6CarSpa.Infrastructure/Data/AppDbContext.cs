...
public class AppDbContext : DbContext
{
 ...
 public DbSet<ShowroomDailyStaff> ShowroomDailyStaff => Set<ShowroomDailyStaff>();
 ...
 // All DateTime columns use "timestamp without time zone" because the app
 // always works with local dates/times. Npgsql rejects Kind=Unspecified
 // against "timestamp with time zone", which happens after JSON round-trips.
 foreach (var prop in b.Model.GetEntityTypes()
 .SelectMany(t => t.GetProperties())
 ...
 e.Property(x => x.ContactPerson).HasMaxLength(120);
 e.Property(x => x.Notes).HasMaxLength(500);
 });

 b.Entity<ShowroomDailyStaff>(e =>
 {
 e.HasIndex(x => new { x.AssignmentDate, x.ShowroomId });
 e.HasIndex(x => x.StaffId);
 e.Property(x => x.AttendanceStatus).HasMaxLength(20).IsRequired();
 e.Property(x => x.Remarks).HasMaxLength(300);
 e.Property(x => x.AssignmentDate).HasColumnType("timestamp without time zone");
 e.Property(x => x.CreatedAt).HasColumnType("timestamp without time zone");
 e.Property(x => x.UpdatedAt).HasColumnType("timestamp without time zone");
 e.HasOne(x => x.Showroom).WithMany(s => s.DailyAssignments)
 .HasForeignKey(x => x.ShowroomId).OnDelete(DeleteBehavior.Cascade);
 e.HasOne(x => x.Staff).WithMany(s => s.ShowroomAssignments)
 .HasForeignKey(x => x.StaffId).OnDelete(DeleteBehavior.Restrict);
 });
